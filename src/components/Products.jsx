  ...
  return (
    <div className='mt-32 relative'>
        {product.map((elem,index)=>(
             <Product key={index} title={elem.title} description={elem.description} live={elem.live} cases={elem.case} mover={mover} count={index}/>
        ))}
         <div className='absolute top-0 w-full h-full pointer-events-none'>
        <motion.div initial={{y:pos, x:"-50%"}} animate={{y:pos+`rem`}} transition={{ease:[0.76,0,0.24,1], duration:0.5}} className='window absolute w-[32rem] h-[18rem] left-[43%] -translate-x-[50%] overflow-hidden'>
